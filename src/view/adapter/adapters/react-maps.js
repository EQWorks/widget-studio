  ...
  adapt: (data, { options, ...config }) => {
    ...
    return ({
      ...
      layerConfig: [{
        ...
        visualizations: {
          radius: config?.mapValueKeys ?
            {
              value: {
                field: `${getKeyValues(mapValueKeys, mapVis.RADIUS)?.key}_${getKeyValues(mapValueKeys, mapVis.RADIUS)?.agg}`,
              },
              valueOptions: [5, 15],
              dataScale: 'linear',
            }:
            null,
          fill: getKeyValues(config?.mapValueKeys, mapVis.FILL)?.key ?
            {
              value: {
                field: `${getKeyValues(mapValueKeys, mapVis.FILL)?.key}_${getKeyValues(mapValueKeys, mapVis.FILL)?.agg}`,
              },
